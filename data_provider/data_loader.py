...
class StockLoader(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        assert flag in ['train', 'test', 'val']
        type_map = {'train': 0, 'val': 1, 'test': 2}
        data_path_map = {'train': 'train-hours.csv', 'val': 'val-hours.csv', 'test': 'test-hours.csv'}
        self.set_type = type_map[flag]
    ...
    #     :return: Last insample window of all timeseries. Shape "timeseries, insample size"
    #     """
    #     insample = np.zeros((len(self.data_x), self.seq_len))
    #     insample_mask = np.zeros((len(self.data_x), self.seq_len))
    #     for i, ts in enumerate(self.data_x):
    #         ts_last_window = ts[-self.seq_len:]
    #         insample[i, -len(ts):] = ts_last_window
    #         insample_mask[i, -len(ts):] = 1.0
    #     return insample, insample_mask
